...
## Goal
Track unread per user per room/DM. Visual badges near room and contact names. Low-latency presence.
...
## Count strategy
Cheap approximation: `count(*) FROM messages WHERE room_id=? AND id > last_message_id`. Cap at 99 for UI. For rooms with 1000 members, per-open-client cost is bounded by cursor scan on indexed column.
