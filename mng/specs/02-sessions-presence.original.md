...
## Goal
Track online / AFK / offline per user across multiple browser tabs and devices. Expose active-sessions screen with per-session logout.
...
| AC-02-02 | AFK: no interaction in any tab for >60s |
| AC-02-03 | Active in ≥1 tab → online |
| AC-02-04 | Offline: no open tab (all tabs closed/unloaded) |
| AC-02-05 | Sessions screen lists browser + IP for each active session |
...
- `GET /api/v1/sessions` → list of sessions for current user
...
2. On WS connect: client sends `sessionId`; BFF registers in `presence:{userId}` with current ts.
