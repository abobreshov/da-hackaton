...
## Goal
Upload images + files up to size limits, attach to messages, access-control by room membership, persist even if uploader loses access.
...
- `GET /api/v1/attachments/:id/download` — streams file, access-controlled
- Attachment ids passed to `POST /messages` body to bind to a message

## Access rules
On download: require `room_memberships(roomId, userId)` exists AND `room_bans` absent; for DM: both participants still friends + no ban.
...
## Risks
- MIME sniffing: enforce allowed MIME vs extension; reject disallowed (e.g. executables) only if policy requires (spec allows arbitrary).
